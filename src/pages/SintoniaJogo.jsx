...
function SintoniaJogo({ setTelaAtual }) {
  ...
  // Função para zerar o placar e os textos
  const resetarJogo = () => {
    const novoAlvo = Math.floor(Math.random() * 130) + 25;
    setAlvoCentro(novoAlvo);
    setAnguloPonteiro(90);
  ...
  // Executa a ação dependendo do que o usuário confirmou
  ...
  return (
    <div className="rules-screen page-transition">
      ...
      {/* PAINEL DE INSTRUÇÕES (Separado da área do jogo para não quebrar o layout) */}
      ...
      <div style={{ display: 'flex', gap: '16px', width: '100%', marginTop: '16px' }}>
        <button className="back-btn" onClick={() => setConfirmacao('resetar')}>RESETAR</button>
        <button className="back-btn" onClick={() => setConfirmacao('sair')}>SAIR</button>
      </div>
